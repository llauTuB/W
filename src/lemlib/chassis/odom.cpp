...
lemlib::Pose lemlib::getPose(bool radians) {
    lemlib::Pose pose = odomPose;

    if (pFilter != nullptr && pFilter->is_enabled()) {
        Pose degPose = pFilter->get_prediction();
        pose = lemlib::Pose(degPose.x, degPose.y, degToRad(degPose.theta));
    }
...
void lemlib::setPose(lemlib::Pose pose, bool radians) {
    if (pFilter != nullptr && pFilter->is_enabled()) {
        float thetaDeg = radians ? radToDeg(pose.theta) : pose.theta;
        pFilter->set_starting_pose(lemlib::Pose(pose.x, pose.y, thetaDeg), true);
    }
    else {
...
void lemlib::update() {
    if (pFilter != nullptr && pFilter->is_initialized()) {
        pFilter->update();
    }
